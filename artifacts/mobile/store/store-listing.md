# DoneMark — App Store & Play Store Listing Content

> Positioning: people reach for DoneMark because they're anxious they *didn't*
...
> That nagging "did I actually do it?" feeling — gone. DoneMark gives you calm,
...
asking "did I take my pill, pay that bill, turn off the stove?" — DoneMark is
for you. It's the calm way to know, with a trusted record, that the things that matter
got done today.

DoneMark isn't another nagging to-do list. It's quiet, dependable proof. Each
day you confirm your routines and DoneMark saves a dated record you can trust.
No more second-guessing. No more circling back to check. Just the steady
confidence that everything is handled.

WHY PEOPLE USE DONEMARK
...
HOW IT WORKS
1. Choose your marks — the routines that matter to you (home safety,
   medication, bills, personal care, pet care, and more).
2. Verify each day — a quick self-confirm, a photo, or a receipt. Whatever
   creates the clearest record.
3. Build your timeline — your proof stacks up into a private record you can
   look back on anytime.

GENTLE DAILY REMINDERS
DoneMark can nudge you once a day, at a time you choose, about anything you
haven't confirmed yet — so nothing slips, without the noise.

BUILT FOR IMPORTANT DAILY ACTIONS
DoneMark was made for people who care deeply about getting it right: caregivers,
people managing medication, bills, pet care, home safety, or the mental load of a household.
...
Stop wondering. Start knowing. Download DoneMark and feel everything fall into
place.
```
...
> (the privacy page can live on the DoneMark web app).

---

## Required URLs

These live on the DoneMark web app (`artifacts/anchored`) as public, no-login
...
> `artifacts/anchored/src/lib/contact.ts` (`support@donemark.app`). Change it to
...
```
DoneMark helps people keep trustworthy daily proof that important routines were
...
user taps "Photo" or "Receipt" on a mark, they can capture a photo or attach
...
Six frames per size, each a device mockup of the real DoneMark UI with a calm,
on-message caption:

1. `01-hero` — "Everything's under control." (today's dashboard)
2. `02-glance` — "See what's done at a glance." (marks, mixed states)
